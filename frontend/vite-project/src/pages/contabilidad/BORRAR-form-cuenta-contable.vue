...
import SeleccionarCuentaContable from './seleccionar-cuenta-contable.vue'; 


// =================================================================
// II. PROPS, EMITS Y FLAGS DE ESTADO INICIAL
// =================================================================
...
const props = defineProps({
    ...
    // NOTA: La descripción ya no se maneja por v-model aquí, 
    // se envía al padre vía evento 'cuentaSeleccionada'.
    
    // Flags de estado
    isMovimientoEnCaja: { type: Boolean, default: false },
    isPolizaBloqueada: { type: Boolean, default: false },
});
...
// =================================================================
// III. COMPUTED PROPERTIES (Manejo de v-model bidireccional)
// =================================================================
...
// =================================================================
// V. MANEJO DE ESTADO EXPUESTO (Comunicación Padre -> Hijo)
// =================================================================

// 1. Función expuesta para limpiar campos (llamada desde el padre)
const limpiarCuentas = () => {
    ...
    c3.value = '';
    c4.value = '';
    // También se limpia la descripción en el padre, pero mediante el evento de la cuenta.
    emit('cuentaSeleccionada', { completa: '', descripcion: '' });
};

// 2. Exponer la función para que el padre pueda llamarla usando ref
defineExpose({
    limpiarCuentas
});
...
// =================================================================
// VI. LÓGICA DE VALIDACIÓN Y BÚSQUEDA
// =================================================================
...
const buscarCuentaContable = async () => {
    ...
    try {
        ...
        // Emitir evento al padre con la descripción. El padre la asignará a descripcionCuentaSP.
        ...
        const errorConfig = (title, message, color) => ({
            visible: true, title, mensaje: message, color,
            confirmText: 'Aceptar', showCancelButton: false, callback: () => {}, 
        });

        if (status === 404) {
          modalConfig.value = errorConfig('Advertencia', 'La cuenta contable no fue encontrada.', 'warning');
        } else {
          modalConfig.value = errorConfig('Error Interno de la Aplicación', 'Ocurrió un error al buscar la cuenta contable.', 'error');
        }
        
        // Emitir evento al padre con descripción vacía en caso de error
...
// =================================================================
// VII. MANEJO DE MODAL DE SELECCIÓN DE CUENTAS
// =================================================================
...
const seleccionarCuentaContableSwal = (data) => {
    if (data) {
        ...
        // Asignar a las propiedades computed (llama al setter / emit de c1-c4)
        c1.value = partes[0] || '';
        c2.value = partes[1] || '';
        c3.value = partes[2] || '';
        c4.value = partes[3] || '';
        ...
        // Emitir la cuenta seleccionada para que el padre la capture
        emit('cuentaSeleccionada', {
        ...
        <VRow>
            <VCol cols="12" md="11">
                ...
                <VCol cols="12" md="3">
                    ...
                    <AppTextField
                        ...
                        :readonly="isMovimientoEnCaja || isPolizaBloqueada"
            ...
            <VCol cols="12" md="1" class="mt-md-5">
                <VBtn 
                    color="primary" 
                    @click="abrirModalCC" 
                    :disabled="isMovimientoEnCaja || isPolizaBloqueada"
                    block
                >
                    ...
                </VBtn>
